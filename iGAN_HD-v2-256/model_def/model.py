...
class BranchGAN(object):
  def __init__(self, sess, input_height=256, input_width=256,
         ...
         use_two_stage_training=False, epoch = 30, 
  ...
  def build_optimizers(self, config):
    ...
    for start_level in range(self.n_levels):
      ...
      #g_vars_lastlayers = self.g_vars_lastlayer_pyramid[start_level]
  ...
  def train(self, config):
    ...
    for level in range(1, self.n_levels, 1):#self.n_levels):  counter//config.epoch+
      ...
      for epoch in xrange(config.epoch):
        ...
        print(isinstance(self.data, list))
        print(self.data[1:3])
  ...
  def discriminator(self, inputs):
      D_pyramid = [None]
      print('discriminator inputs pyramid: ',inputs)
  ...
  def sub_discriminator(self, input_tensor, start_level):
    with tf.variable_scope("discriminator", reuse=False) as scope:
      h_pyramid = []
      print('sub_discriminator input tensor: ',input_tensor)
