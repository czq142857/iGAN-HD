...
class IM2Z(object):
  ...
  def find_best(self, z0, image=None, mask=None, edge_map=None):
    ...
    while np.abs(value - value_o) > 1e-4:
      ...
      #print(value)
    img_best, err_c, err_e = self.sess.run([self.G_best, self.loss_c, self.loss_e], feed_dict=fdict)
    print('[End] optimizing z, time spent ' + '{}'.format(time.time()-time_start))
    return img_best, err_c, err_e

  def predict(self, color_map = None, mask = None, edge_map=None):
    err_c, err_e= 0.0, 0.0
    ...
    out_img, err_c, err_e = self.find_best(z0, image=color_map, mask=mask, edge_map=edge_map)
    return out_img
