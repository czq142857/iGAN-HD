...
def tf_hog_descriptor(images, cell_size = 8, block_size = 2, block_stride = 1, n_bins = 9,
                      grayscale = False, oriented = False, type='tensor'):

    if type == 'tensor':
      batch_size, height, width, depth = images.get_shape().as_list()
      img = images
    else:
      batch_size, height, width, depth = images.shape
      img = tf.constant(images, name="ImgBatch", dtype=tf.float32)
    print(batch_size, height, width, depth, img.dtype)
    ...
    for i in range(n_bins):
        ...
        hist.append(tf.reduce_sum(tf.where(mask, cell_norm, zero), 3))
